...
## TIM as the three natural Probe modes
...
It needs a **default Geometry Extraction** that runs after (or as part of) every Interaction:

1. Interaction Signal (or internal Think/Mature event) occurs.
2. Geometry Extraction produces a **Geometry Receipt** (relative Mass proxies, Tension components, DoF/Constraints, Jurisdiction shifts, Stem differentials).
...
- **Geometry Receipt** is the local (and optionally consented) interpretation of that signal under the observer's Metric Stem.
- **Tensor** is the live geometric reading of Registry alloys under the Metric Stem (`docs/tensor.md`). Geometry Receipts are the primary way the Tensor is updated.
...
- That every prompt already produces perfect geometry. Extraction quality is an engineering surface that will improve.
