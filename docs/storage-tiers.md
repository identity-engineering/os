...
| Tier | Storage | Character |
|------|---------|-----------|
| **Free** | Local (YAML and/or SQLite) | Full ownership, offline-first, "Setz dir das einfach auf" |
| **Personal Pro** | IE-managed SQL (start: Supabase) | Device-independent, backups, central truth, authenticated skill access |
...
3. **Free remains local-first.** No account, no cloud dependency, full Ownership of the files.
4. **Personal Pro is a managed convenience and continuity layer**, not a change of the geometric model. The user still owns the data; IE operates the infrastructure, backups and multi-device access.
...
- Free: YAML files as human/agent-readable source of truth; optional local SQLite for speed.
- Personal Pro: Supabase (Postgres) with the same tables; Row Level Security + authenticated access; skill uses the managed connection.
