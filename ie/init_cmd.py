...
def _patch_header(header_path: Path, handle: str, preferred_name: Optional[str]) -> None:
    ...
    if yaml is not None:
        ...
        return
    # Minimal string replace fallback if PyYAML missing
    lines = []
...
def init_install(
    ...
    *,
    ...
    # Ensure foreign-estimate zone dir exists
    fe = target / "registry" / "_foreign_estimates"
    fe.mkdir(parents=True, exist_ok=True)

    _patch_header(target / HEADER_NAME, handle=handle, preferred_name=preferred_name)

    # Light README for the install root
    readme = target / "README.md"
    if not readme.exists() or force:
        readme.write_text(
            f"# IE install — `{handle}`\n\n"
            f"Created by `ie init`.\n\n"
